# adapted from iocaine powder
# source: https://www.kaggle.com/group16/rps-roshambo-comp-iocaine-powder/notebook
...
class Predictor:
    ...
    def bestguess(self, age, best):
        bestdiff = self.stats.max(
            age, (best[0] - self.prediction) % 3, best[1])
...
class Iocaine:
    ...
    def move(self, them):
        ...
        for meta in range(len(ages)):
            best = self.predict_meta[meta].bestguess(
                len(self.histories[0]), best)
...
def player(prev_play):
    ...
    if not prev_play:
        ...
        act = iocaine.move(mapRPS[prev_play])

    act = reverse_mapRPS[act]
    return act
